...
def _judge(cur: float | None, prev: float | None,
    ...
    上期基数 ≤ 0（亏损转盈利、财务费用由净收益转净支出）或低于地板时不判同比：
    前者同比在数学上成立但没有经济含义，后者会算出「同比 -11088.9%」这种让人
    不敢信整张表的数字（实测来自上期 0.09 亿 / 本期 -9.89 亿）。这两种情况下
    变动金额本身就是异动信号，过地板即入榜。
    """
    if prev is None:
        return None, False
...
def build(raw: dict) -> dict | None:
    ...
    return {
        ...
        "门槛说明": (
            ...
            "这类靠百分比挤进榜、对报表毫无影响的噪音。上期基数 ≤ 0 或低于地板时不判同比"
            "（亏损转盈利、基数过小），只看变动金额。每组按变动金额取前几名，组内序号即重要性排序"
        ),
