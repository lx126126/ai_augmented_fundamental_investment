...
{{
  ...
  "lynch_type": "彼得林奇六类之一。**必须从下面 6 个值里原样选一个，一个字都不要改**：周期型 / 快速成长 / 稳健成长 / 缓慢增长 / 困境反转 / 资产富余。不要加「型」字、不要加括号、不要加后缀",
  "lynch_note": "可选补充：支持该分类判断的关键依据（≤10 字，如 高股息现金牛 / 大盘国有银行 / 资源一体化），**没有就填空字符串**。分类本身不要写在这里",
...
6. `lynch_type` 必须**逐字照抄**上面给的 6 个值之一：「稳健成长」不要写成「稳健增长型」，「快速成长」不要写成「快速增长型」。分类依据一律写进 `lynch_note`，不要混在 `lynch_type` 里。
7. {_NA_RULE}"""
