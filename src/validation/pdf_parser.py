...
PDF_FIELD_ALIASES = {
    ...
    "net_profit_parent": [
        ...
        # 银行口径：归属说明放在括号里（交行「净利润（归属于母公司股东）」）
        "净利润（归属于母公司股东）", "净利润(归属于母公司股东)",
    ],
    "ocf": ["经营活动产生的现金流量净额", "经营活动现金流量净额"],
    "total_equity": [
        "归属于本公司股东的净资产", "归属于上市公司股东的净资产",
        "归属于母公司股东的净资产", "归属于母公司所有者权益",
        # 银行口径：交行「股东权益（归属于母公司股东）」
        "股东权益（归属于母公司股东）", "股东权益(归属于母公司股东)",
    ],
...
# 单位 → 换算到「元」的乘数
# ⚠️「千元」是 A 股年报最常见的利润表单位之一（美的 2025 即用），漏登记的后果是
#    解析值整体小 1000 倍，且**静默**（见 _UNIT_PATTERNS 的说明）。
_UNIT_MULTIPLIER = {
    "元": 1,
    "千元": 1e3,
    "万元": 1e4,
...
def _match_field(name: str) -> str | None:
    ...
    匹配顺序很关键：**先用完整名匹配，再去掉括号后缀兜底**。
    银行（如交行）把归属说明写在括号里——「净利润（归属于母公司股东）」，若先去括号就
    退化成「净利润」（含少数股东口径），会与归母口径混淆，所以必须先试完整名。
    """
    name = _clean(name)
    for field, aliases in PDF_FIELD_ALIASES.items():
        if name in aliases:
            return field
    stripped = re.sub(r"[（(][^）)]*[）)]", "", name)  # 去「（元）」等括号后缀
    if stripped != name:
        for field, aliases in PDF_FIELD_ALIASES.items():
...
# 单位识别：按「确定性」从高到低排列，命中即返回。
# 注意「百万元」必须排在「万元」前面——否则正则里「万元」会先吃掉「百万元」的后半段。
#
# 🔴 前缀里**绝不能**再用 `[^，。\n]*?`（曾经用过）。惰性前缀会让引擎逐字符右移，
#    于是「单位：千元」在「千」处不匹配、右移一格后命中裸「元」→ 识别成「元」，
#    数值整体小 1000 倍。**这比匹配失败更危险**：匹配失败会退回默认值并留下痕迹，
#    而「匹配成功但单位错」全程无异常，只有校验差异才能看出来（2026-09-18 美的 000333 实测）。
#    所以前缀只允许「人民币」这种固定词，不允许任意字符。
_UNIT_PATTERNS = (
    r"单位[:：]\s*(?:人民币)?\s*(百万元|千元|万元|亿元|元)",        # 单位：人民币百万元 / 单位：千元
    r"单位为\s*(?:人民币)?\s*(百万元|千元|万元|亿元|元)",           # (除特别注明外，金额单位为人民币千元)
    # 惰性前缀：若用贪婪 {0,20}，引擎会先吃满再回溯，在「…百万元」的「万元」处提前命中，
    # 永远轮不到「百万元」→ 银行年报（百万元）被误判成万元，差 100 倍。
    r"[（(][^（）()]{0,20}?(百万元|千元|万元|亿元)[^（）()]{0,6}[）)]",  # （除另有标明外，人民币百万元）
    r"人民币\s*(百万元|千元|万元|亿元)",                          # 人民币百万元
)
...
def _detect_unit(page_text: str) -> str:
    ...
    兼容格式：
    - 「单位：百万元」「金额单位：人民币百万元」「单位：千元」（显式标注）
    - 「（除另有标明外，人民币百万元）」——**银行年报（如交行 601328）用这种括号夹注**，
      页面里根本没有「单位」二字，旧实现三处匹配全落空 → 退回默认「元」，把 265071 百万元
      当成 265071 元，校验直接算出 99999900% 的假差异（数据其实完全一致）。
    - 「(除特别注明外，金额单位为人民币千元)」——「单位为」无冒号写法（美的 000333）。
    """
