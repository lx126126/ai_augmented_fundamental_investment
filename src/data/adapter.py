...
PIE_GROUPS = [
    ("营业总成本", "total_operating_cost", [
        ...
        # 🔴 资产减值损失 / 信用减值损失**故意不列在这里**。
        #    2019 年新金融工具准则起，这两个科目从「二、营业总成本」的项下**移到了
        #    营业总成本之外**（单列在总成本与营业利润之间）。实测伊利 2026H1：
        #    营业总成本 551.34 亿 = 上述六项之和，**不含**资产减值 24.56 亿
        #    （见 2026 半年报合并利润表）。
        #    列进来的后果：正项加总 > 大类总额 → 占比分母偏大、每项占比都被稀释，
        #    而饼图仍正常渲染、看不出错。这两行在修复 `fields.IMPAIRMENT_FIELDS`
        #    之前一直是空值（从未生效），字段一补上就会立刻失真，所以同步移除。
    ]),
    ("流动资产", "current_assets", [
        ("货币资金", "monetary_funds"),
        ("拆出资金", "lend_fund"),
