...
def _swing_block(review: dict) -> str:
    ...
    for i, e in enumerate(items):
        ...
        rows.append(
            ...
            + f'<td class="op-num">{_op_yoy(e.get("同比_pct"), e.get("同比口径"))}</td>'
            + "</tr>"
        )
    table = (
    ...
    # 「同比」的分母是 |上期|，且跨越零点时不给百分比 —— 这两类都必须在脚注里点明：
    # 不点明，「+92.0%」会被读成「大增 92%」（而它是流出收窄），空着的格子会被当成「没数据」。
    # 只在榜上**真有**这类行时才加 —— 每只标的都挂一句与它无关的口径注，
    # 等于训练读者跳过脚注，真需要读的那次也不会读了。
    kinds = {e.get("同比口径") for e in items if e.get("同比口径")}
    if kinds & {"上期为负", "由负转正"}:
        # 不再重复「分母取 |上期|」—— 那句已在 `口径说明` 里，脚注里说三遍只会显得啰嗦。
        notes.append(
            "上期为净流出的科目，其同比的正号表示净额改善（流出收窄 / 转为净流入）、"
            "负号表示恶化，与「变动」列同向"
        )
    cross = [e["名称"] for e in items
             if e.get("同比口径") in ("由负转正", "由正转负")]
    if cross:
        notes.append(
            f"其中 {'、'.join(cross)} 跨越零点（上期与本期符号相反），不给百分比 —— "
            "分母过小会算出失真比值，看「本期 / 变动」两列即可"
        )
...
def _op_yoy(v, tag: str | None = None) -> str:
    """同比着色：涨红跌绿（A 股习惯），并显式带正负号。

    跨越零点（`tag` 为「由负转正 / 由正转负」）时**不给百分比**，直接写这个事实：
    分母过小会算出失真比值（实测茅台投资活动现金流 -3.10 亿 → +253.1 亿 =
    +8265.9%，没人会信，还会连累读者怀疑整张表）。用「由负转正」而不是「转正」——
    后者对财务费用这类科目听起来像好事，而这张表不做利好利空判断。
    """
    if tag in ("由负转正", "由正转负"):
        return f'<span class="op-na">{tag}</span>'
    if v is None:
