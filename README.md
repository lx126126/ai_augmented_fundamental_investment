...
| **数仓设计** | parquet 文件存储（schema 化 PostgreSQL/DuckDB 表设计推进中） | 🔄 |
| **管道编排** | 数据质量检验 + 告警已内置；Airflow DAG 调度推进中 | 🔄 |
| **LLM 应用层** | DeepSeek 结构化输出（JSON schema 约束），生成商业模式 / 投资逻辑 / 风险 / 林奇分类；铁律「只翻译数据，不编数」 | ✅ |
| **后端服务** | FastAPI 查询 API 推进中（当前为离线脚本 + 静态报告输出） | 🔄 |
...
.
├── src/
│   ├── data/          # 数据层：fetcher（拉取）/ cleaner（清洗）/ fields（字段映射）/ adapter（宽表→模板）/ storage（入库）
...
| P1 | 数据层：多源接入 + 文档解析 + 清洗入库 + 数据验证 | ✅（剩：全市场批量拉取、Airflow 调度 🔄） |
| P2 | 分析层：估值分位 / 格雷厄姆体检 / 造假检测 / 竞争地位 | ✅ |
| P3 | 报告层：数据绑定 + LLM 叙事 + 机构评级 + 业务版图 | ✅（剩：FastAPI 查询 API 🔄、季度更新引擎） |
| P4 | 产品层：小程序 + 内容发布 | ⬜ |
