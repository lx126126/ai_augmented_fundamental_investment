...
### 1.4b 宏观数据层 `src/macro/`（2026-09-23 新增）

与个股链路**并行**的第二条数据链：回答「现在整个市场处在什么位置」，而非「这家公司怎么样」。
两者只共享网络出口配置与重试约定（`src.data.fetcher` / `src.data.retry`），
**数据形态完全不同** —— 个股是结构化财报表（按报告期对齐、上游会重述），
宏观是时间序列（日/月/季混杂、官方口径极少回填、接口每次回传全量历史）。

| 模块 | 行数 | 职责 |
|---|---|---|
| `macro/indicators.py` | 825 | **指标定义唯一真源**：47 个指标的展示名 / 单位 / 频率 / 数据源 / 口径 / 变化量口径 / 是否参与周期定位。**新增指标只改这里**，渲染端不动 |
| `macro/source.py` | 578 | AKShare 接口 → 标准化时间序列（期间归一化 + 排序 + 去 NaN）+ 单接口墙钟超时 + 单次构建内缓存 |
| `macro/derive.py` | 238 | 序列 → 展示视图（值 / 上期 / 变化量 / 历史分位 / 迷你走势）；派生指标算法（滚动 4 季 GDP、ERP、按共同期间对齐相减相除） |
| `macro/clock.py` | 148 | 周期定位：增长 × 通胀四象限，**规则透明可复算**；方向分歧如实报，不硬塞象限 |
| `macro/build.py` | 303 | 编排：拉取 → 派生 → payload（单指标失败降级但不静默 + **数据过期自检** + 构建预算 + 数值格式化） |
| `scripts/build_macro.py` | 509 | → `web/macro.html`（**入库的构建产物**，与 `web/index.html` 同约定）；`--json` 导出 payload / `--from-json` 跳过拉数 |
| `scripts/build_xhs_macro.py` | 618 | → 9 图小红书发布包（复用 `build_xhs` 的版式层，内容层全新）；与看板**共用同一份 payload**（`--from-json`） |
| `scripts/check_caption_numbers.py` | 84 | 校对工具：逐项核对**小红书正文里的每个数字都能在图卡上找到出处** |
| `tests/test_macro.py` | 630 | **77 个单测，全部不联网**（构造数据），覆盖期间归一化 / 派生算法 / 象限判定 / 指标表完整性 / 渲染烟雾 / 展示文案清洁度 |

**四条铁律**（写在 `macro/__init__.py`，改代码前先看）：
1. 每个指标**带自己的数据日期** —— 宏观发布时点差异极大（M2 到 8 月、社融只到 4 月、行情到 9-22），混排即误导；
2. **变化量有三种口径** —— 收益率用 `bp`、占比指标用 `pct`（百分点）、其余用 `%`（相对变化率），不可混用；
3. **色阶只表示变动方向，不表示利好利空** —— 失业率上行与用电量上行同色，含义却相反；
4. **展示文案是纯文本、不是 Markdown** —— 渲染端是 HTML，写 `**重点**` 会原样显示星号且**任何一层都不报错**
   （实测漏过 3 处，全是翻页面才发现的）。`tests/test_macro.py` 有一组断言专门扫这件事。

**调度**：`scripts/macro_refresh.sh` + `com.fqf.macro-refresh.plist`（**周六 10:00**，周闸门）。
有意**不并入日更**：看板 47 个指标里绝大多数是月频/季频，日更只会把同一份月度数据重复渲染 22 次。
